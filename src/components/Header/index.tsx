        ...
        <div className="container">
          <div className="relative -mx-4 flex items-center justify-between">
            <div className="w-60 max-w-full px-4 xl:me-12">
              <Link
                href="/"
                className={`block w-full text-2xl font-extrabold ${
            ...
            <div className="flex w-full items-center justify-between px-4">
              <div className="-ms-24 flex flex-1 justify-center xl:-ms-10">
                <button
                  onClick={navbarToggleHandler}
                  id="navbarToggler"
                  aria-label="Mobile Menu"
                  className="absolute left-4 top-1/2 block -translate-y-1/2 rounded-lg px-3 py-[6px] ring-primary focus:ring-2 lg:hidden"
                >
                  <span
                    ...
                    {menuData.map((menuItem, index) => (
                      <li key={index} className="group relative">
                        {menuItem.path ? (
                          <Link
                            href={menuItem.path}
                            className={`flex py-2 text-base lg:ms-0 lg:inline-flex lg:px-0 lg:py-6 ${
                            ...
                            >
                              {menuItem?.submenu?.map((submenuItem, index) => (
                                <Link
                                  ...
                                  key={index}
                                  className="block rounded px-3 py-2.5 text-sm text-dark hover:text-primary dark:text-white/70 dark:hover:text-white"
                                >
                                  {submenuItem.title}
                                </Link>
                              ))}
              ...
              <div className="flex items-center justify-end pe-16 lg:pe-0">
                <Link
                  href="#"
                  className="hidden items-center gap-2 px-7 py-3 text-base font-medium text-dark hover:opacity-70 dark:text-white md:flex "
                >
                  English
